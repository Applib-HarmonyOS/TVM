 ...
 */
final class Base {
  private static final String TAG = Base.class.getName();
  ...
  static {
    ...
    try {
      ...
      LogUtil.warn(TAG,"[WARN] TVM native library not found in path. "
      ...
      try {
        ...
        LogUtil.warn(TAG,"[WARN] Couldn't find native library tvm4j.");
        e.printStackTrace();
        LogUtil.warn(TAG,"Try to load tvm4j (runtime packed version) ...");
        try {
          ...
          LogUtil.info(TAG,"tvm4j_runtime_packed loads successfully.");
        } catch (UnsatisfiedLinkError errFull) {
          LogUtil.error(TAG,"[ERROR] Couldn't find native library tvm4j_runtime_packed.");
          throw new TVMRuntimeException(errFull.getMessage());
        }
      }
    }

    LogUtil.info(TAG,"libtvm4j loads successfully.");
        ...
        try {
          ...
          NativeLibraryLoader.extractResourceFileToTempDir(runtimeLibname, new Action() {
            @Override public void invoke(File target) {
              LogUtil.info(TAG,"Loading tvm runtime from " + target.getPath());
              checkCall(_LIB.nativeLibInit(target.getPath()));
            }
          });
        } catch (IOException e) {
          throw new TVMRuntimeException(e.getMessage());
        }
      }
    } else {
  ...
  private static void tryLoadLibraryOS(String libname) throws UnsatisfiedLinkError {
    try {
      LogUtil.info(TAG,String.format("Try loading %s from native path.", libname));
  ...
  private static void tryLoadLibraryXPU(String libname, String arch) throws UnsatisfiedLinkError {
    LogUtil.info(TAG, String.format("Try loading %s-%s from native path.", libname, arch));
